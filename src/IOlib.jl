...
function supplyusedata(param_file::String)
    ...
    if params["report-diagnostics"]
        writedlm("diagnostics/qs.csv",  qs, ',')
        writedlm("diagnostics/M.csv",  M, ',')
        writedlm("diagnostics/marg.csv",  margins, ',')
		writedlm("diagnostics/import_frac.csv",  retval.m_frac, ',')
        writedlm("diagnostics/qd.csv",  vec(sum(use_table, dims=2)), ',')
        writedlm("diagnostics/qd_alt.csv",  intermed, ',')
        writedlm("diagnostics/X.csv",  retval.X, ',')
        writedlm("diagnostics/F.csv",  retval.F, ',')
        writedlm("diagnostics/I.csv",  retval.I, ',')
        writedlm("diagnostics/g.csv", retval.g, ',')
        writedlm("diagnostics/g_alt.csv", retval.S * qs, ',')
        writedlm("diagnostics/qd_alt2.csv", retval.D * retval.g, ',')
        writedlm("diagnostics/S.csv", retval.S, ',')
        writedlm("diagnostics/D.csv", retval.D, ',')
		open("diagnostics/nonenergy_energy_link_measure.txt", "w") do fhndl
