...
function supplyusedata(param_file::String)
    ...
    M[params["non-tradeable-range"]] = [0.0 for i in params["non-tradeable-range"]] # If it is declared non-tradeable, set imports to zero
	retval.M = M
    ...
    X[params["non-tradeable-range"]] = [0.0 for i in params["non-tradeable-range"]] # If it is declared non-tradeable, set exports to zero
	retval.X = X
